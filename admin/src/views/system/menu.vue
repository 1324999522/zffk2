<template>
  <div>
    ...
    <el-card>
      ...
      <div>
        ...
        <el-table :data="form" style="width: 100%" stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="title" label="菜单名称" min-width="90">
          </el-table-column>
          <el-table-column prop="component" label="组件路径" min-width="90">
          </el-table-column>
          <el-table-column prop="path" label="路由地址" min-width="90">
          </el-table-column>
          <el-table-column prop="icon" label="icon" min-width="90">
          </el-table-column>
          <!-- 表单操作区域 -->
    ...
    <!-- 弹出框【增】 -->
    <el-dialog title="添加客户" :visible.sync="isCreate" width="400px" :close-on-click-modal="false" @close="close()">
      <menuUpdate :id="0"> </menuUpdate>
    </el-dialog>
    <!-- 弹出框【改】 -->
    <el-dialog title="编辑资料" :visible.sync="isEdit" width="400px" :close-on-click-modal="false" @close="close()">
      <menuUpdate :id="id" :key="id"> </menuUpdate>
    </el-dialog>
  </div>
</template>
...
import menuUpdate from './menu_update'
